...
if set == 1 or set == 2:
    ...
    s = ",".join(s)
    t2 = f'''curl -X POST -H "Content-Type: application/json" -d '{{ "param": [{s}] }}' http://127.0.0.1:5001/simulation/loadStream'''
elif set == 3: # 纯转发
    insId = 23023
    ...
    t2 = '''curl -X POST -H "Content-Type: application/json" -d '{ "param": [{ "insId": 23001, "startTime": 2000, "endTime": 100000000, "source": 0, "destination": 1, "bizType": "6"  } ] }'  http://127.0.0.1:5001/simulation/loadStream'''
    ...
    t2 = '''curl -X POST -H "Content-Type: application/json" -d '{ "param": [{ "insId": 1003, "startTime": 2000, "endTime": 10000000, "source": 0, "destination": 1, "bizType": "11"}, { "insId": 1004, "startTime": 2000, "endTime": 10000000, "source": 0, "destination": 1, "bizType": "12" }, { "insId": 1005, "startTime": 2000, "endTime": 10000000, "source": 0, "destination": 1, "bizType": "13" } ] }' http://127.0.0.1:5001/simulation/loadStream'''

elif set == 6: # 短消息
    t2 = '''curl -X POST -H "Content-Type: application/json" -d '{ "param": [{ "insId": 23001, "startTime": 2000, "endTime": 100000000, "source": 0, "destination": 1, "bizType": "3"  } ] }'  http://127.0.0.1:5001/simulation/loadStream'''
