...
def multi_flow_biz_config(biz_comp, task_num, biz_data, start_time, end_time):
    for i in range(0, int(task_num * biz_comp.weight)):
        # 生成一对收发端id
...
# 业务泊松分布
def biz_config(biz_comp, task_num, biz_data, start_time, end_time, biz_duration=180, max_biz=3):
    # 记录业务起始时间（队列）
    biz_starttime = []
    # 记录不同用户的业务发起时间
    ue_starttime = {}
    ...
    while sum_time < duration:
        ...
        if sum_time > duration:
            break
        elif ifcollide(start_time, max_biz, biz_starttime, sum_time, biz_duration, source_id, dest_id, ue_starttime):
            biz_starttime.append(start_time + sum_time)
            ue_starttime[str(source_id)].append(start_time + sum_time)
            tmpdict = {
                'startTime': biz_starttime[-1] * 1000,
                'endTime': min(biz_starttime[-1] + biz_duration, end_time) * 1000,
                'source': source_id,
                'destination': dest_id,
                'bizType': biz_comp.bizType
            }
